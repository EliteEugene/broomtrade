...
from dajngo.views.generic.detail import DetailView
...
class BlogCreate(PermissionRequiredMixin, SuccessMessageMixin, CreateView, CategoryListMixin):
...
class BlogDelete(PermissionRequiredMixin, PageNumberView, TemplateView, SearchMixin, PageNumberMixin):
    ...
    def get(self, request, *args, **kwargs):
        self.blog = Blog.objects.get(pk=self.kwargs["pk"])
        if self.blog.user = request.user or request.user.is_superuser:
            return super(BlogDelete, self).get(requesr, *args, **kwargs)
        else:
            return redirect(reverse("login"))
